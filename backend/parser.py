...
class IPNParser:
    ...
    def getKeyWordsForPhrase(self,phrase):
        ...
        numOfKeywords = 100
        # print(allTexts)
        e = yake.KeywordExtractor(lan=language, n=max_ngram_size, dedupLim=deduplication_threshold, dedupFunc=deduplication_algo, windowsSize=windowSize, top=numOfKeywords, features=None)
            ...
            for sentence in allTexts.split("."):
                if extractedPhrase[0] in sentence:
                    if extractedPhrase[0] in phrasesWithSentces:
                        phrasesWithSentces[extractedPhrase[0]]['sentences'].append(sentence.replace('\xa0',' '))
                    else:
                        phrasesWithSentces[extractedPhrase[0]]= {'value':extractedPhrase[1],'sentences':[sentence.replace('\xa0',' ')]}
